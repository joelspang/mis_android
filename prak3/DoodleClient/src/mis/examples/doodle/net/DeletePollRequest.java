...
public class DeletePollRequest extends SpringAndroidSpiceRequest<Poll> {
	...
	try {
	    ...
	    // Add an intercepter for custom headers (we need to add the doodle
	    // key)
	    t.setInterceptors(new ArrayList<ClientHttpRequestInterceptor>());
	    t.getInterceptors()
		    .add(new DeletePollDoodleKeyIntercepter(this.poll
			    .getDoodleKey()));

	    // Send the DELETE request
	    t.delete(MainActivity.APIUrl + "polls/" + this.poll.getId(),
		    this.poll, String.class);
    ...
    class DeletePollDoodleKeyIntercepter implements
